...
export default function ArmadaScreen() {
  ...
  const [selectedVehicleType, setSelectedVehicleType] = useState("Bus");
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.headerRow}>
          ...
          <View style={styles.headerCopy}>
            <Text style={styles.title}>Jadwal Armada</Text>
            <Text style={styles.subtitle}>Kelola armada, rute, tarif, dan jadwal dalam satu halaman</Text>
          </View>
        </View>
            ...
            {activeForm === "travel" ? (
              <>
                ...
                <View style={styles.choiceRow}>
                  {['Bus', 'Minibus', 'Travel'].map((vehicleType) => (
                    <TouchableOpacity
                      key={vehicleType}
                      style={[styles.choiceChip, selectedVehicleType === vehicleType && styles.choiceChipActive]}
                      onPress={() => setSelectedVehicleType(vehicleType)}
                    >
                      <Text style={[styles.choiceText, selectedVehicleType === vehicleType && styles.choiceTextActive]}>{vehicleType}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
                <Text style={styles.formLabel}>Nama Armada</Text>
                <TextInput style={styles.input} placeholder="Nama armada" value={travelForm.nama_travel} onChangeText={(value) => setTravelForm((current) => ({ ...current, nama_travel: value }))} />
                <TextInput style={styles.input} placeholder="Nomor plat" value={travelForm.nomor_kendaraan} onChangeText={(value) => setTravelForm((current) => ({ ...current, nomor_kendaraan: value }))} />
        ...
        <DashboardCard title="Daftar Armada" subtitle="Kelola armada mitra">
          ...
          {travels.map((item) => (
            <View key={item.id} style={styles.armadaCard}>
              <View style={styles.armadaMain}>
                {item.foto ? (
                  <Image source={{ uri: item.foto }} style={styles.armadaImage} />
                ) : null}
                <View style={styles.infoBody}>
                  <View style={styles.rowHeader}>
                    <View style={styles.armadaBadge}>
                      <Text style={styles.armadaBadgeText}>{selectedVehicleType || "Bus"}</Text>
                    </View>
                    <Text style={styles.rowTitle}>{item.name}</Text>
                  </View>
                  <Text style={styles.rowMeta}>Plat {item.nomorKendaraan || "-"}</Text>
                  <Text style={styles.rowMeta}>{item.jumlahKursi || 0} kursi • Status {item.status}</Text>
                </View>
              </View>
...
const styles = StyleSheet.create({
  ...
  rowBody: { flex: 1 },
  armadaCard: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 12, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: "#F1F5F9" },
  armadaMain: { flexDirection: "row", alignItems: "center", flex: 1, gap: 12 },
  armadaImage: { width: 72, height: 72, borderRadius: 14, backgroundColor: "#E2E8F0" },
  infoBody: { flex: 1 },
  rowHeader: { flexDirection: "row", alignItems: "center", gap: 10, marginBottom: 6, flexWrap: "wrap" },
  armadaBadge: { backgroundColor: "#DBEAFE", paddingHorizontal: 10, paddingVertical: 6, borderRadius: 999 },
  armadaBadgeText: { color: "#1E3A8A", fontSize: 12, fontWeight: "700" },
